...
NO_WRAP:MOV     DX,[BX]                 ;Char in DX now
       ...
       ;NOTE: Your program takes over here (keep in mind that DS is still at 
       ;ROM_BIOS_DATA segment). The just-struck key's scan code is in DH and 
       ;its ASCII code in DL at this point. If you want to remove this key from 
       ;the keyboard buffer (i.e. accept it as input), use the instruction MOV 
       ;TAIL,BX here. Otherwise, you may exit by jumping to the label EXIT.

        ;MOV    TAIL,BX         ;<--Optional removal of key from buffer.
        ;
        ;Your code here.
        ;
...
;--------------------------------------------------
xyz:
         MOV AX,DX                      ;COPY OVER THE KEY CODE
         CMP AL,0                       ; V KEY PROBLEMS
         JNE GOXYZ                      ;
         JMP DO_OMEGA                   ;
GOXYZ:
...
NOR:     CMP CS:STRESS_FLAG,1              ;IS THE STRESS FLAG ON?
         ...
         JE  UMLAUT
         MOV DX,AX                      ;CONTINUE BY ADJUSTING SI
         MOV DH,0
         MOV SI,DX
         CMP SI,128                     ;ARE WE ABOVE 128?
         JL  CONTY                      ;YES, NO TRANSLATION REQUIRED
         JMP EXIT
CONTY:   MOV AL,CS:GREEK_TABLE[SI]         ;NO, TRANSLATE CHARACTER AND LEAVE
         MOV [BX],AX
         JMP EXIT
...
STRESS:  MOV CS:STRESS_FLAG,0              ;RESET THE STRESS FLAG
         ...
         JMP EXIT
CONTY1:  MOV AL,CS:GREEK_STRESS_TABLE[SI]  ;NO, TRANSLATE CHARACTER AND LEAVE
         MOV [BX],AX
         JMP EXIT
